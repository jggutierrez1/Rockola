...
SCAN FOR !EMPTY(file02.MARK)
	...
	IF FILE(sFile1)
		sCadena="("+ALLTRIM(STR(iCount))+" de "+ALLTRIM(STR(iTot))+"):. "
		WAIT WINDOW "Borrando carátula "+sCadena+"["+sFile1+"]" NOWAIT
		DELETE FILE (sFile1)
		=Write_TO_Histd(oFs,"Borrando carátula "+sCadena+"["+sFile1+"]")
	ELSE
		WAIT WINDOW "["+sFile1+"] no encontrado..." NOWAIT
		=Write_TO_Histd(oFs,"["+sFile1+"] no encontrado...")
	ENDIF
...
FOR igCnt=1 TO igTot-1
	...
	IF !FOUND()
		...
		WAIT WINDOW "GÉNERO CREADO: ["+sCadena1+"]..." TIMEOUT 1 
	ELSE
		WAIT WINDOW "GÉNERO ANEXADO A : ["+ALLTRIM(tmp_1.DESCRI)+"]..." TIMEOUT 1 
	ENDIF
	DIMENSION aDis(1)
	idTot=ADIR(aDis,"&sGenDir\*.*","D")
	IF idTot<=0 THEN
		WAIT WINDOW "No se encontraron discos en este género ["+sGenDir+"], SERÀ IGNORADO:..." TIMEOUT 2
		LOOP
	ENDIF
	FOR idCnt=3 TO idTot
		...
		IF !FOUND()
			...
			REPLACE;
				...
				ULT_ACT WITH DATETIME(),;
				FL_PRD	WITH 0
			WAIT WINDOW "Copiando carátula de disco:.."+ALLTRIM(sFleCar) NOWAIT
		...
		FOR icCnt=1 TO (iTot_MP3)
			...
			IF !FOUND()
				...
				WAIT WINDOW "Copiando archivo de música:.. "+ALLTRIM(sFile)+" ("+STR((icCnt/iTot_MP3)*100,6,2)+" %)" NOWAIT
...
SCAN
	...
	GO TOP
	WAIT WINDOW NOWAIT;
		"Aplicando Ordenación a los todos los discos del género [" +;
